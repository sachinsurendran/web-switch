...
uint16_t print_webpage(uint8_t *buf, byte on_off)
{
       ...
       int i=0;
        ...
        plen=es.ES_fill_tcp_data_p(buf,0,PSTR("HTTP/1.0 200 OK\r\nContent-Type: text/html\r\n\r\n"));
        plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<center><p><h1><a href=\"http://www.hobbyist.co.nz\">\"www.hobbyist.co.nz\"</a> presents WebSwitch! </h1></p> "));
         
        plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<hr><br><form METHOD=get action=\""));
        plen=es.ES_fill_tcp_data(buf,plen,baseurl);
        plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("\">"));
        plen=es.ES_fill_tcp_data_p(buf,plen,PSTR("<h2> KETTLE is  </h2> "));
        if (on_off){
